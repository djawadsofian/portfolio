...
const SkillCard = ({ skill }: { skill: { name: string; category: string; img: string } }) => {
  ...
  return (
    <div className="w-28 sm:w-40 aspect-square cursor-default">
      <div className="relative w-full h-full bg-black/30 backdrop-blur-sm border border-yellow-400/20 rounded-2xl overflow-hidden p-4 flex flex-col items-center justify-center">
        <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-3 ${needsWhiteBg ? "bg-white" : "bg-white/10"}`}>
          <img 
            src={skill.img} 
            alt={skill.name} 
...
const ScrollingRow = ({ skills, direction }: { skills: Array<{ name: string; category: string; img: string }>, direction: "left" | "right" }) => {
  ...
  const { skills1, skills2, categorySummary } = useMemo(() => {
    ...
    return {
      ...
      categorySummary: [
        ...
        { category: "Mobile", count: (categoryCounts.Mobile || 0) + (categoryCounts.DevOps || 0) }
      ]
    };
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          {categorySummary.map((item) => (
            ...
            >
              <div className="text-2xl font-bold text-yellow-400 mb-1">{item.count}+</div>
